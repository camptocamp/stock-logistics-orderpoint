...
class TestStockOrderpointSafetyStockFixtureSerie01(OrderpointSafetyStockCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env.company.safety_stock_history_days = 69
    ...
    def test_fixture_serie_01_with_days_to_order(self):
        with self._mock_days_to_order(5):
            self.orderpoint.action_apply_safety_stock()
            self.assertRecordValues(
                self.orderpoint,
                [
...
class TestStockOrderpointSafetyStockScenarios(OrderpointSafetyStockCommon):
    ...
    def test_scenario_product_sold_every_10_days(self):
        ...
        """
        self.env.company.safety_stock_history_days = 360
        ...
        # Now let's say lead time is 4 days, but we have 8 days to order (buffer)
        self.orderpoint.rule_ids.delay = 4
        self.orderpoint.invalidate_recordset()
        with self._mock_days_to_order(8):
            self.orderpoint.action_apply_safety_stock()
            self.assertRecordValues(
                self.orderpoint,
                [
            ...
            [
                {
                    ...
                    "product_max_qty": 40,  # Days to order is zero, so max == min
                }
            ],
        )
        # Now let's say we have 7 days to order (buffer)
        with self._mock_days_to_order(7):
            self.orderpoint.invalidate_recordset()
            self.orderpoint.action_apply_safety_stock()
            self.assertRecordValues(
                self.orderpoint,
